...
const MyListingsPage = () => {
    ...
    const handleUpdate = async () => {
        try {
            ...
            const result = await res.json();
            if (res.ok) {
                setListings(prev => prev.map(item => item._id === editData._id ? editData : item));
                setEditData(null);
                        ...
                        <tbody>
                            {listings.map((listing, index) => (
                                <tr key={listing._id} className="border-b border-base-200 hover:bg-base-200/50 transition text-center">
                                    ...
                                    <td className="p-4 align-middle text-neutral">{listing.category}</td>
                                    <td className="p-4 align-middle">
                                        <div className="flex justify-center items-center gap-1 text-neutral">
                                            <FaDollarSign className="text-secondary" /> {listing.price}
                                        </div>
                                    </td>
                                    <td className="p-4 align-middle">
                                        <div className="flex justify-center items-center gap-2 text-neutral">
                ...
                <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[1000]">
                    <div className="bg-base-100 p-8 rounded-2xl w-full max-w-md shadow-xl border border-base-200 text-center">
                        ...
                        <p className="text-neutral mb-6">
                            Are you sure you want to delete listing <span className="font-bold">{deleteId}</span>?
                        </p>
